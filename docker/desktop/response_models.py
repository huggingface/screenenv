...
class DirectoryNode(BaseModel):
    ...
    children: Optional[List['DirectoryNode']] = Field(default=None, description="List of child nodes for directories")
    error: Optional[str] = Field(default=None, description="Error message if the node couldn't be accessed")
...
class WindowSizeResponse(BaseResponse):
    ...
    window_id: Optional[int] = Field(default=None, description="Window identifier")
...
class DesktopPathResponse(BaseResponse):
    ...
    is_writable: bool = Field(..., description="Whether the desktop directory is writable")
...
class CursorPositionResponse(BaseResponse):
    ...
    screen: int = Field(default=0, description="Screen number (for multi-monitor setups)")
...
class AccessibilityTreeResponse(BaseResponse):
    ...
    platform: str = Field(..., description="Platform-specific accessibility implementation used")
...
class RecordingResponse(BaseResponse):
    ...
    duration: Optional[float] = Field(default=None, description="Duration of the recording in seconds")
    size: Optional[int] = Field(default=None, description="Size of the recording file in bytes")
