...
logger = app.logger
recording_process = (
    None  # fixme: this is a temporary solution for recording, need to be changed to support multiple-process
)
...
libreoffice_version_tuple: tuple[int, ...] | None = None
MAX_DEPTH = 50
MAX_WIDTH = 1024
MAX_CALLS = 5000
...
def _create_atspi_node(node: Accessible, depth: int = 0, flag: str | None = None) -> _Element:
    node_name = node.name
...
def _create_pywinauto_node(node, nodes, depth: int = 0, flag: str | None = None) -> _Element:
    nodes = nodes or set()
    if node in nodes:
        return
    nodes.add(node)
    ...
    if children:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_child = [
                executor.submit(_create_pywinauto_node, ch, nodes, depth + 1, flag) for ch in children[:MAX_WIDTH]
            ]
        try:
...
def get_accessibility_tree():
    ...
    if os_name == "Linux":
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            futures = [executor.submit(_create_axui_node, wnd, None, 0) for wnd in foreground_windows + dock_info]
...
def activate_window():
    ...
    if os_name == "Windows":
        ...
        subprocess.run(["wmctrl", "-{:}{:}a".format("x" if by_class_name else "", "F" if strict else ""), window_name])
...
def close_window():
    ...
    if os_name == "Windows":
        ...
        subprocess.run(["wmctrl", "-{:}{:}c".format("x" if by_class_name else "", "F" if strict else ""), window_name])
    elif os_name == "Darwin":
        import pygetwindow as gw
